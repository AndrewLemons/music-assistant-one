...
func render(size: Int, mac: Bool, to url: URL) throws {
    // iOS App Store icons must be opaque; macOS icons retain transparent margins.
    let channels = mac ? 4 : 3
    ...
    NSGradient(
        starting: NSColor(red: 0.98, green: 0.32, blue: 0.40, alpha: 1),
        ending: NSColor(red: 0.76, green: 0.09, blue: 0.26, alpha: 1)
    ...
    mark.curve(
        to: NSPoint(x: 430, y: 210),
        controlPoint1: NSPoint(x: 643, y: 200),
        controlPoint2: NSPoint(x: 482, y: 163)
    )
    mark.curve(
        to: NSPoint(x: 440, y: 359),
        controlPoint1: NSPoint(x: 341, y: 260),
        controlPoint2: NSPoint(x: 365, y: 339)
    )
    mark.curve(
        to: NSPoint(x: 545, y: 356),
        controlPoint1: NSPoint(x: 478, y: 370),
        controlPoint2: NSPoint(x: 519, y: 369)
    )
    mark.close()
    mark.fill()
