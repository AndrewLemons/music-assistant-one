...
actor SendspinStateStore: PairingRecordStore, SendspinPersistenceProvider {
    ...
    init(onFailure: @escaping @Sendable (String) -> Void) throws {
        ...
        if let data = try Self.load() {
            ...
            let candidate = Snapshot(identitySecret: SendspinIdentity.generate().secretKeyBytes,
                                     pairingPsk: Psk.generate().bytes, records: [])
        ...
        guard state.version == 1, let identity = SendspinIdentity(secretKeyBytes: state.identitySecret),
              let psk = Psk(bytes: state.pairingPsk), state.records.allSatisfy({ $0.value != nil }) else {
    ...
    nonisolated var pairingToken: String { PairingToken(clientKey: identity.publicKeyBytes, pairingPsk: pairingPSK).string }
    func checkHealth() throws { if let storageFailure { throw MAError.message(storageFailure) } }
    func listRecords() async -> [PairingRecord] { snapshot.records.compactMap(\.value) }
    func insert(_ record: PairingRecord) async throws {
        guard record.pskId != pairingPSK.pskId, record.pskId != Psk.sentinel.pskId,
              !snapshot.records.contains(where: { $0.value?.pskId == record.pskId }) else { throw PairingRecordStoreError.duplicatePskId }
    ...
    func markUsed(pskId: String) async {
        guard let index = snapshot.records.firstIndex(where: { $0.value?.pskId == pskId }), !snapshot.records[index].used else { return }
    ...
    func loadManagementConfiguration(default configuration: PairingManagementConfiguration) async -> PairingManagementConfiguration {
        PairingManagementConfiguration(pairingPsk: pairingPSK, pairingPskEnabled: true,
    ...
    func storageAccounting() async -> PairingStorageAccounting? {
        PairingStorageAccounting(free: max(0, 16 - snapshot.records.count), capacity: 16, costIndividual: 1, costShared: 1)
    }
    func loadLastPlayedServerId() async -> String? { snapshot.lastPlayedServerId }
