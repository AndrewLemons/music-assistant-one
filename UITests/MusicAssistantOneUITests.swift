...
final class MusicAssistantOneUITests: XCTestCase {
    ...
    #if os(macOS)
    @MainActor
    func testLibraryFilterSurvivesNowPlaying() throws {
        let app = XCUIApplication()
        app.launchArguments = ["--demo"]
        app.launch()
    ...
    @MainActor
    func testSearchFiltersAndStablePlayer() throws {
        let app = XCUIApplication()
        app.launchArguments = ["--demo"]
        app.launch()
        XCTAssertTrue(app.buttons["miniPlayer"].waitForExistence(timeout: 10))
        ...
        #if os(macOS)
        XCTAssertEqual(playerY, app.buttons["miniPlayer"].frame.midY, accuracy: 2,
